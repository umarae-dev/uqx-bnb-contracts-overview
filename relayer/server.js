...
if (!addressPattern.test(FACTORY_ADDRESS)) {
  throw new Error("SAFECORE_FACTORY_ADDRESS must be configured with the deployed V4 factory address.");
}
if (!/^0x[0-9a-fA-F]{64}$/.test(RELAYER_PRIVATE_KEY)) {
  throw new Error("SAFECORE_RELAYER_PRIVATE_KEY must be supplied through the private runtime environment.");
}
if (!Number.isSafeInteger(PORT) || PORT < 1 || PORT > 65535) throw new Error("Invalid PORT.");
if (!Number.isFinite(RATE_LIMIT_PER_MINUTE) || RATE_LIMIT_PER_MINUTE < 1 || RATE_LIMIT_PER_MINUTE > 600) {
  throw new Error("Invalid SAFECORE_RELAYER_RATE_LIMIT.");
}
...
function clientIp(req) {
  // Trust only the right-most X-Forwarded-For hop supplied by the platform.
  // Operators should strip client-supplied forwarding headers at the HTTPS edge.
...
function checkRateLimit(req) {
  ...
  if (rate.size > 10_000) {
    for (const [key, values] of rate) {
      if (!values.some((t) => t >= windowStart)) rate.delete(key);
    }
  }
  return true;
}
...
async function relay(target, data) {
  return withRelayNonceLock(async () => {
    ...
    // Simulate the exact signed call before spending relayer gas. Invalid,
    // expired, replayed or over-budget signatures revert here and are rejected.
    ...
    if (balance < maxCost) throw new Error("relayer_insufficient_bnb");
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.url === "/relay") {
      ...
      const { target, data } = normalizeRelayRequest(body);
      const txHash = await relay(target, data);
      return json(res, 202, { tx_hash: txHash, chain_id: Number(CHAIN_ID) });
    }

    return json(res, 404, { error: "not_found" });
  } catch (error) {
    // Never log request calldata: emergency rescue calldata contains a one-time
    // recovery secret. The chain sees it only when the signed rescue executes.
...
async function boot() {
  ...
  console.log(`SafeCore V4 relayer ready on chain ${CHAIN_ID}.`);
