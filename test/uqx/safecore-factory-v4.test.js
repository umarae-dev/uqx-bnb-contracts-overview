...
describe("SafeCoreFactoryV4", function () {
  ...
  async function fixture() {
    ...
    const assets = [NATIVE];
    const limits = [ethers.parseEther("1")];
    return { factory, identity, device, safe1, safe2, relayer, attacker, commitment, assets, limits };
  }
  ...
  it("lets a relayer deploy without receiving any account authority", async function () {
    const { factory, identity, device, safe1, safe2, relayer, commitment, assets, limits } = await fixture();
    const delay = 2 * 24 * 60 * 60;
    const configHash = await factory.configurationHash(device.address, safe1.address, safe2.address, commitment, delay, assets, limits);
    ...
    await factory.connect(relayer).createAccountFor(
      identity.address, device.address, safe1.address, safe2.address, commitment, delay,
      assets, limits, deadline, sig
    );
  ...
  it("rejects config tampering by the relayer", async function () {
    const { factory, identity, device, safe1, safe2, relayer, attacker, commitment, assets, limits } = await fixture();
    const delay = 2 * 24 * 60 * 60;
    const configHash = await factory.configurationHash(device.address, safe1.address, safe2.address, commitment, delay, assets, limits);
    const deadline = BigInt((await time.latest()) + 3600);
    const sig = await createSignature(factory, identity, configHash, 0n, deadline);

    await expect(factory.connect(relayer).createAccountFor(
      identity.address, device.address, attacker.address, safe2.address, commitment, delay,
      assets, limits, deadline, sig
