...
contract SafeCoreAccountV4_1 is ReentrancyGuard, EIP712 {
    ...
    error Unauthorized(); error ZeroAddress(); error ZeroAmount(); error InvalidSignature(); error SignatureExpired(); error InvalidDelay();
    error AlreadyAuthorized(); error NotAuthorized(); error LastDevice(); error EnrollmentMissing(); error PairingMismatch(); error InvalidRecoverySecret();
    error RecoveryNotArmed(); error RetiredAccount(); error AccountNotRetired(); error InvalidSuccessorConfig(); error EmergencyDestinationOnly();
    error InvalidEmergencyDestinations(); error DestinationChangeMissing(); error DestinationChangeNotReady(); error ArrayLengthMismatch(); error TooManyItems();
    error InsufficientBalance(); error BudgetExceeded(); error BudgetNotReduced(); error BudgetUnchanged(); error BudgetIncreaseMissing();
    error BudgetIncreaseNotReady(); error DuplicateAsset(); error AmountTooLarge(); error NativeTransferFailed(); error DeviceRegistryCorrupt();
    ...
    function applyBudgetIncrease(address asset) external onlyActiveAccount {
        ...
        b.limit = p.newLimit; delete pendingBudgetChange[asset]; emit BudgetIncreaseApplied(asset, oldLimit, p.newLimit);
    }
    ...
    function _transferAsset(address asset, address payable to, uint256 amount) private {
        if (asset == NATIVE_ASSET) { if (amount > address(this).balance) revert InsufficientBalance(); (bool ok,) = to.call{value: amount}(""); if (!ok) revert NativeTransferFailed(); }
        else { IERC20 token = IERC20(asset); if (amount > token.balanceOf(address(this))) revert InsufficientBalance(); token.safeTransfer(to, amount); }
    }
    function _addAuthorizedDevice(address device) private { if (device == address(0)) revert ZeroAddress(); if (authorizedDevice[device]) revert AlreadyAuthorized(); authorizedDevice[device] = true; _authorizedDevices.push(device); _authorizedDeviceIndexPlusOne[device] = _authorizedDevices.length; authorizedDeviceCount = _authorizedDevices.length; }
    function _removeAuthorizedDevice(address device) private { uint256 indexPlusOne = _authorizedDeviceIndexPlusOne[device]; if (indexPlusOne == 0 || !authorizedDevice[device]) revert DeviceRegistryCorrupt(); uint256 index = indexPlusOne - 1; uint256 lastIndex = _authorizedDevices.length - 1; if (index != lastIndex) { address moved = _authorizedDevices[lastIndex]; _authorizedDevices[index] = moved; _authorizedDeviceIndexPlusOne[moved] = index + 1; } _authorizedDevices.pop(); delete _authorizedDeviceIndexPlusOne[device]; authorizedDevice[device] = false; authorizedDeviceCount = _authorizedDevices.length; }
    ...
    function _consumeBudget(address asset, uint256 amount) private { if (recoveryCommitment == bytes32(0)) revert RetiredAccount(); if (amount == 0) revert ZeroAmount(); if (amount > type(uint192).max) revert AmountTooLarge(); _rollEpoch(asset); Budget storage b = _budgets[asset]; uint256 next = uint256(b.spent) + amount; if (next > b.limit) revert BudgetExceeded(); b.spent = uint192(next); }
